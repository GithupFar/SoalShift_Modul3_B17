...
void* kepiting(void *arg)
{
	while (kepistat >0 && kepistat <=100){
		if (kepistat<=0 || kepistat>101 || lohstat<=0 || lohstat >101) pthread_exit (&kepistat);
		kepistat -= 10;
...
void* lohan (void *arg)
{
	while (lohstat >0 && lohstat <=100){
		if (kepistat<=0 || kepistat>101 || lohstat<=0 || lohstat >101) pthread_exit (&lohstat);
		lohstat -= 15;
...
int main(){
	...
	while (1)
	{
		if (kepistat < 1 || lohstat < 1 || kepistat >101 || lohstat > 101){
			printf("Mati semua!\n");
			pthread_cancel (tid[0]);
			pthread_cancel (tid[1]);
			break;
		}
		printf("===== Game kasih-makan kepiting & lohan =====\n");
		printf("Menu: \n1. Lihat Status\n2. Kasih Makan\n");
		scanf("%d",&menu);
		if (menu == 1) printf("Status Kepiting = %d \n Status Lohan = %d\n",kepistat,lohstat);
		else if (menu == 2)
		{
			printf("\n1.Kepiting\n2.Lohan\n");
